# ThirdSpace CodeBuddy Code 入口

## Bootstrap（新用户 / 任意机器）

**当 CodeBuddy Code 在 vault 根目录（含 `.thirdspace/workspace-index.yaml` 的目录）打开时，按顺序读取：**
...
3. CodeBuddy Code Stop Hook 安装（`~/.codebuddy/hooks/session-stop.sh`）
4. Skill 全局注册（`~/.codebuddy/skills/thirdspace-vault` 软链）
