...
4. Call `reminder.list` with `status: "incomplete"` and no completion bounds. Then call it separately with `status: "completed"`, `completionStart` equal to the exact local-day start, and `completionEnd` equal to the exact next-day start. The Bridge applies that `[00:00, next 00:00)` range in EventKit's completed-Reminder predicate; do not fetch unbounded completion history and filter it locally.
