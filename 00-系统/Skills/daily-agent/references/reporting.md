...
Use `monthly` for both report commands to generate a monthly review. `report-aggregate` also supports a bounded custom interval with `--start YYYY-MM-DD --end YYYY-MM-DD`; `review-generate` accepts only `weekly` or `monthly`.
...
`review-generate` reads that bounded input and updates only the managed block in `02-日记/复盘/`. User text outside the managed markers is preserved. Weekly and monthly runs update only their matching timestamp in `agent-state.json`.
