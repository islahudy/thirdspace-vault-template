...
用户当天第一次主动打开 Agent，并表达“开始今天”“今日计划”“遗留事项”或“阅读积压”等意图时，加载 `daily-agent` Skill；如本机可用，同时加载 `eventkit` Skill 以获取当日 Calendar 和 Reminders 的新鲜上下文。`eventkit` 不可用时仍继续完整的本地开场流程。先运行每日开场回顾，等待用户确认旧事项状态和 1～3 个今日重点，再完成计划快照。
